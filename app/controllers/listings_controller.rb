 	  ...
 	  return_time_range_begin = search_params["return_time_range_begin"]
	     ...
	     listing.comments = listing.comments + "*("+ ((detoured_travel_time_object[0] - direct_travel_time_object[0])/60).to_s + " mins)*" 
            ...
            if listing_type == "offer"
             ...
             if !((depart_time_range_begin+first_leg_time >= listing.depart_range_start and depart_time_range_begin+first_leg_time <= listing.depart_range_end) or (depart_time_range_end+first_leg_time >= listing.depart_range_begin and depart_time_range_end+first_leg_time <= listing.depart_range_end))
                listing.comments = listing.comments + " OUT OF TIME RANGE. first leg takes (mins): " + (first_leg_time/60).to_s
                puts listing.depart_range_start
                puts depart_time_range_begin
                puts listing.depart_range_start+first_leg_time
